...
fn InterfaceFunc(Self: type) type {
    return struct {
...
const ActualInterface = Interface.Init(InterfaceFunc);
...
const SomeStruct = struct {
    ...
    pub fn func1(self: *SomeStruct) void {
        std.debug.print("Hello World {}\n", .{self.val});
    }

    pub fn func2(self: *SomeStruct, number: isize) void {
        std.debug.print("Number: {} {}\n", .{ self.val, number });
    }

    pub fn actualInterface(self: *SomeStruct) ActualInterface {
        return ActualInterface.init(self);
    }
};

pub fn main() !void {
    var someStruct = SomeStruct{ .val = 10 };
